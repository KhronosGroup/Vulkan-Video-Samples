...
VkResult VkVideoEncoderH265::InitEncoderCodec(VkSharedBaseObj<EncoderConfig>& encoderConfig)
{
    ...
    if (result != VK_SUCCESS) {
        LOG_S_ERROR << "InitEncoder() failed with ret: " << result << std::endl;
        return result;
    }
    ...
    result = m_vkDevCtx->CreateVideoSessionParametersKHR(*m_vkDevCtx,
                                                         ...
                                                         &sessionParameters);
    if(result != VK_SUCCESS) {
        LOG_S_ERROR << "\nEncodeFrame: Failed to get create video session parameters" << std::endl;
        return result;
    }
    ...
    if(result != VK_SUCCESS) {
        LOG_S_ERROR << "EncodeFrame: Failed to get create video session object." << std::endl;
        return result;
    }
                                        ...
                                        uint32_t frameIdx, uint32_t ofTotalFrames)
{
    ...
    // naluSliceSegmentEntryCount == intraRefreshCycleDuration on the refreshing
    // frame (VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10852). The cycle duration was
    ...
    pFrameInfo->pictureInfo.naluSliceSegmentEntryCount =
