        ...
        VkRenderPass renderPass, VkImageView depthView)
{
    ...
    VkResult result = m_vkDevCtx->CreateImageView(*m_vkDevCtx, &viewCreateInfo, nullptr, &mImageView);
    ...
    return m_vkDevCtx->CreateFramebuffer(*m_vkDevCtx, &fbCreateInfo, nullptr, &mFramebuffer);
}
...
VkResult VulkanSyncPrimitives::CreateSyncPrimitives(const VulkanDeviceContext* vkDevCtx)
{
    ...
    VkResult result = m_vkDevCtx->CreateFence(*m_vkDevCtx, &fenceCreateInfo, nullptr, &mFence);
    if (result != VK_SUCCESS) {
        return result;
    }
    ...
    semaphoreCreateInfo.pNext = nullptr;
    semaphoreCreateInfo.flags = 0;
    return m_vkDevCtx->CreateSemaphore(*m_vkDevCtx, &semaphoreCreateInfo, nullptr, &mRenderCompleteSemaphore);
}
...
VkResult VulkanRenderPass::CreateRenderPass(const VulkanDeviceContext* vkDevCtx, VkFormat displayImageFormat)
{
    ...
    return m_vkDevCtx->CreateRenderPass(*m_vkDevCtx, &renderPassCreateInfo, nullptr, &renderPass);
}

// Create Graphics Pipeline
VkResult VulkanGraphicsPipeline::CreatePipeline(const VulkanDeviceContext* vkDevCtx, VkViewport* pViewport, VkRect2D* pScissor,
        VkRenderPass renderPass, VulkanDescriptorSetLayout* pBufferDescriptorSets)
{
    ...
    if (m_cache == VkPipelineCache(0)) {
        ...
        CALL_VK(m_vkDevCtx->CreatePipelineCache(*m_vkDevCtx, &pipelineCacheInfo, nullptr, &m_cache));
    }
    ...
    DestroyPipeline();
    VkResult pipelineResult = m_vkDevCtx->CreateGraphicsPipelines(*m_vkDevCtx, m_cache, 1,
                                                                  &pipelineCreateInfo,
                                                                  nullptr, &m_pipeline);
                                                   ...
                                                   const VulkanVertexBuffer& vertexBuffer)
{
    ...
    VkResult result = m_vkDevCtx->BeginCommandBuffer(cmdBuffer, &cmdBufferBeginInfo);
    if (result != VK_SUCCESS) {
        return result;
    }
    ...
    if (scaleInput) {
        if (displayWidth && (displayWidth != inputImageToDrawFrom->imageWidth)) {
            constants.texMatrix[0] = vk::Vec2((float)displayWidth / (float)inputImageToDrawFrom->imageWidth, 0.0f);
        }

        if (displayHeight && (displayHeight != inputImageToDrawFrom->imageHeight)) {
            constants.texMatrix[1] = vk::Vec2(.0f, (float)displayHeight / (float)inputImageToDrawFrom->imageHeight);
        }
    }
    ...
    return m_vkDevCtx->EndCommandBuffer(cmdBuffer);
}
