...
inline VkResult WaitAndResetFence(const VkInterfaceFunctions* vkIf, VkDevice device, VkFence fence,
    ...
    while (fenceTotalWaitTimeout >= fenceCurrentWaitTimeout) {
        ...
        if (result == VK_TIMEOUT) {
            LOG_WARN("\t **** fence  %s(%llu) is not done after %llu mSec with result 0x%x ****\n",
    ...
    if (result != VK_SUCCESS) {
        LOG_ERROR("\t **** fence  %s(%llu) is not done after %llu mSec with result 0x%x ****\n",
    ...
    if (resetAfterWait) {
        ...
        if (result != VK_SUCCESS) {
            LOG_ERROR("ResetFences() result: 0x%x\n", result);
...
inline VkResult WaitAndGetStatus(const VkInterfaceFunctions* vkIf, VkDevice device, VkFence fence,
    ...
    do {
        ...
        if (result != VK_SUCCESS) {
            ...
            LOG_ERROR("GetQueryPoolResults() result: 0x%x\n", queryResult);
...
inline bool IsValidFilePath(const char *pFilePath, bool checkExists = true) {
    if (!pFilePath || pFilePath[0] == '\0') {
        LOG_S_ERROR << "File path is null or empty" << std::endl;
        return false;
    }
    ...
    try {
        ...
        if (checkExists) {
            if (!std::filesystem::exists(path)) {
                LOG_S_ERROR << "File does not exist: " << pFilePath << std::endl;
                return false;
            }

            if (std::filesystem::is_directory(path)) {
                LOG_S_ERROR << "Path is a directory, not a file: " << pFilePath << std::endl;
                return false;
            }

            if (!std::filesystem::is_regular_file(path)) {
                LOG_S_ERROR << "Path is not a regular file: " << pFilePath << std::endl;
                return false;
            }
        } else {
            ...
            if (!parent.empty() && !std::filesystem::exists(parent)) {
                LOG_S_ERROR << "Parent directory does not exist: " << parent << std::endl;
                return false;
            }
        }
        ...
        LOG_S_ERROR << "Invalid file path (" << e.what() << "): " << pFilePath << std::endl;
        return false;
    }
}
