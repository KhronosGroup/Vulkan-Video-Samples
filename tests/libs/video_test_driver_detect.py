...
class DriverMapping:
    ...
    # See: https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkDriverId.html
    DRIVER_ID_TO_NAME = {
        1: "amd",                    # VK_DRIVER_ID_AMD_PROPRIETARY
        2: "amd",                    # VK_DRIVER_ID_AMD_OPEN_SOURCE
        3: "mesa_radv",              # VK_DRIVER_ID_MESA_RADV
        4: "nvidia",                 # VK_DRIVER_ID_NVIDIA_PROPRIETARY
        5: "intel",                  # VK_DRIVER_ID_INTEL_PROPRIETARY_WINDOWS
        6: "intel",                  # VK_DRIVER_ID_INTEL_OPEN_SOURCE_MESA (ANV)
        ...
        25: "imagination_open",      # VK_DRIVER_ID_IMAGINATION_OPEN_SOURCE_MESA
...
def parse_driver_from_output(stdout: str, stderr: str = "") -> Optional[str]:
    ...
    Looks for the line:
    "*** Selected Vulkan physical device with name: ... vendor ID: 0xXXXX ... device ID: 0xXXXX ***"
    ...
    Returns:
        Detected driver name (e.g., "nvidia", "radv", "anv") or None if not found
    ...
    # Pattern to match the device selection line
    # Looks for: vendor ID: 0xXXXX ... device ID: 0xXXXX
    pattern = r'vendor ID:\s*0x([0-9a-fA-F]+).*?device ID:\s*0x([0-9a-fA-F]+)'
    ...
    try:
        ...
        # Use parse_driver_info with fallback to vendor ID
...
def parse_driver_info(vendor_id: int,
    ...
    Args:
        ...
        driver_id: Optional Vulkan driver ID from VkPhysicalDeviceDriverProperties
