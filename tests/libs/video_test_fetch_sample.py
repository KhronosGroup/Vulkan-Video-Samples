...
class FetchableResource:
    ...
    def __post_init__(self):
        ...
        self.actual_checksum = None  # Track actual checksum if verification failed
        ...
        while True:
            ...
            if current_time - last_update_time >= 0.1 or downloaded == total_size:
                ...
                if elapsed > 0:
                    ...
                    progressbar = f'[{"=" * filled}{"-" * (40 - filled)}]'
                    stats = f'{mb_info} | {rate / (1024*1024):.2f} MB/s'
                    print(f'\r{progressbar} {progress * 100:.1f}% {stats} | ETA: {eta_str}',
                          end='', flush=True)
        ...
        if self.failed_downloads:
            ...
            for resource in self.failed_downloads:
                ...
                if resource.actual_checksum:
                    algo_name = resource.checksum_algorithm.upper()
                    print(f"  Expected {algo_name}: {resource.checksum}")
                    print(f"  Actual {algo_name}:   {resource.actual_checksum}")
