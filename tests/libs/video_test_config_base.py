...
def load_skip_list(skip_list_path: Optional[str] = None) -> List[SkipRule]:
    ...
    Args:
        skip_list_path: Path to skip list JSON file.
                       If None, uses default 'skipped_samples.json' in tests dir.
    ...
    if skip_list_path is None:
        ...
        skip_list_path = str(Path(__file__).parent.parent / "skipped_samples.json")
...
def is_test_skipped(
    ...
    for rule in skip_rules:
        ...
        if not fnmatch.fnmatch(test_name, rule.name):
            continue
