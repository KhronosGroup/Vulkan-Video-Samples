    ...
    for (mask = pStd->refresh_frame_flags; mask; mask >>= 1) {
        if (mask & 1) {
            ...
            for (uint8_t refName = STD_VIDEO_AV1_REFERENCE_NAME_LAST_FRAME; refName < STD_VIDEO_AV1_NUM_REF_FRAMES; refName ++) {
                ...
                m_pBuffers[ref_index].RefFrameSignBias[refName] = (uint8_t)GetRelativeDist(pStd->OrderHint, ref_order_hint);
            }
...
bool VulkanAV1Decoder::ParseObuSequenceHeader()
{
    ...
    sps->flags.still_picture = u(1) & 0x1;
    sps->flags.reduced_still_picture_header = u(1) & 0x1;
    ...
    if (sps->flags.reduced_still_picture_header) {
        ...
        sps->flags.frame_id_numbers_present_flag = u(1) & 0x1;
    }
    ...
    sps->flags.use_128x128_superblock = u(1) & 0x1;
    sps->flags.enable_filter_intra = u(1) & 0x1;
    sps->flags.enable_intra_edge_filter = u(1) & 0x1;
    ...
    if (sps->flags.reduced_still_picture_header) {
        ...
        sps->flags.enable_interintra_compound = u(1) & 0x1;
        sps->flags.enable_masked_compound = u(1) & 0x1;
        sps->flags.enable_warped_motion = u(1) & 0x1;
        sps->flags.enable_dual_filter = u(1) & 0x1;
        sps->flags.enable_order_hint = u(1) & 0x1;
        if (sps->flags.enable_order_hint) {
            sps->flags.enable_jnt_comp = u(1) & 0x1;
            sps->flags.enable_ref_frame_mvs = u(1) & 0x1;
        } else {
            sps->flags.enable_jnt_comp = 0;
            sps->flags.enable_ref_frame_mvs = 0;
        }
    ...
    sps->flags.enable_superres = u(1) & 0x1;
    sps->flags.enable_cdef = u(1) & 0x1;
    sps->flags.enable_restoration = u(1) & 0x1;
    // color config
    ...
    sps->color_config.flags.mono_chrome = sps->seq_profile != STD_VIDEO_AV1_PROFILE_HIGH ? u(1) & 0x1 : 0;
    sps->color_config.flags.color_description_present_flag = u(1) & 0x1;
    ...
    if (sps->color_config.flags.mono_chrome) {
        sps->color_config.flags.color_range = u(1) & 0x1;
        ...
        sps->color_config.flags.separate_uv_delta_q = u(1) & 0x1;
    }
    sps->flags.film_grain_params_present = u(1) & 0x1;
...
void VulkanAV1Decoder::SetupFrameSize(int frame_size_override_flag)
{
    ...
    //render size
    pStd->flags.render_and_frame_size_different = u(1) & 0x1;
...
bool VulkanAV1Decoder::ReadFilmGrainParams()
{
    ...
    if (sps->flags.film_grain_params_present && (pic_data->showFrame || showable_frame)) {
		pStd->flags.apply_grain = u(1) & 0x1;
        ...
        pFilmGrain->flags.update_grain = pStd->frame_type == STD_VIDEO_AV1_FRAME_TYPE_INTER ? u(1) & 0x1 : 1;
        ...
        pFilmGrain->flags.chroma_scaling_from_luma = !sps->color_config.flags.mono_chrome ? u(1) & 0x1 : 0;
        ...
        pFilmGrain->flags.overlap_flag = u(1) & 0x1;
        pFilmGrain->flags.clip_to_restricted_range = u(1) & 0x1;
    } else {
        memset(pFilmGrain, 0, sizeof(StdVideoAV1FilmGrain));
    }
...
bool VulkanAV1Decoder::DecodeTileInfo()
{
	...
	pTileInfo->flags.uniform_tile_spacing_flag = u(1) & 0x1;
    ...
    if (pTileInfo->flags.uniform_tile_spacing_flag) {
        ...
        for (uint32_t off = 0, i = 0; off < sb_cols; off += tile_width_sb)
            pic_data->MiColStarts[i++] = (uint16_t)off;

        pic_data->tileInfo.TileCols = (uint8_t)((sb_cols + tile_width_sb - 1) / tile_width_sb);
        ...
        for (uint32_t off = 0, i = 0; off < sb_rows; off += tile_height_sb)
            pic_data->MiRowStarts[i++] = (uint16_t)off;

        pic_data->tileInfo.TileRows = (uint8_t)((sb_rows + tile_height_sb - 1) / tile_height_sb);
        ...
        pic_data->width_in_sbs_minus_1[tile_col] = (uint16_t)(sb_cols - (pic_data->tileInfo.TileCols - 1) * tile_width_sb - 1);

        uint32_t tile_row = 0;
         for ( ; tile_row < pic_data->tileInfo.TileRows - 1u; tile_row++)
            pic_data->height_in_sbs_minus_1[tile_row] = (uint16_t)(tile_height_sb - 1);
        pic_data->height_in_sbs_minus_1[tile_row] = (uint16_t)(sb_rows - (pic_data->tileInfo.TileRows - 1) * tile_height_sb - 1);

        // Derivce superblock column / row start positions
        uint32_t i, start_sb;
        for (i = 0, start_sb = 0; start_sb < sb_cols; i++) {
            pic_data->MiColStarts[i] = (uint16_t)start_sb;
            start_sb += tile_width_sb;
        }
        pic_data->MiColStarts[i] = (uint16_t)sb_cols;

        for (i = 0, start_sb = 0; start_sb < sb_rows; i++) {
            pic_data->MiRowStarts[i] = (uint16_t)start_sb;
            start_sb += tile_height_sb;
        }
        pic_data->MiRowStarts[i] = (uint16_t)sb_rows;
    } else {
...
void VulkanAV1Decoder::DecodeQuantizationData()
{
    ...
    pic_data->quantization.DeltaQYDc = (uint8_t)ReadDeltaQ(6);
    if (!sps->color_config.flags.mono_chrome) {
        int diff_uv_delta = 0;
        if (sps->color_config.flags.separate_uv_delta_q)
            diff_uv_delta = u(1);
        pic_data->quantization.DeltaQUDc = (uint8_t)ReadDeltaQ(6);
        pic_data->quantization.DeltaQUAc = (uint8_t)ReadDeltaQ(6);
        if (diff_uv_delta) {
            pic_data->quantization.DeltaQVDc = (uint8_t)ReadDeltaQ(6);
            pic_data->quantization.DeltaQVAc = (uint8_t)ReadDeltaQ(6);
        } else {
    ...
    pic_data->quantization.flags.using_qmatrix = u(1) & 0x1;
...
void VulkanAV1Decoder::DecodeSegmentationData()
{
    ...
    flags->segmentation_enabled = u(1) & 0x1;
    ...
    if (pStd->primary_ref_frame == STD_VIDEO_AV1_PRIMARY_REF_NONE) {
        ...
        flags->segmentation_update_map = u(1) & 0x1;

        if (flags->segmentation_update_map) {
            flags->segmentation_temporal_update = u(1) & 0x1;
        } else {
            flags->segmentation_temporal_update = 0;
        }

        flags->segmentation_update_data = u(1) & 0x1;
    }
    ...
    if (flags->segmentation_update_data) {
        for (int i = 0; i < STD_VIDEO_AV1_MAX_SEGMENTS; i++) {
            ...
            for (int j = 0; j < STD_VIDEO_AV1_SEG_LVL_MAX; j++) {
                ...
                pSegmentation->FeatureData[i][j] = (uint16_t)feature_value;
            }
        }
    } else {
        if (pStd->primary_ref_frame != STD_VIDEO_AV1_PRIMARY_REF_NONE) {
...
void VulkanAV1Decoder::DecodeLoopFilterdata()
{
    ...
    pLoopFilter->flags.loop_filter_delta_enabled = u(1) & 0x1;
...
bool VulkanAV1Decoder::ParseObuFrameHeader()
{
    ...
    if (sps->flags.reduced_still_picture_header) {
        ...
        pic_flags->error_resilient_mode = (pStd->frame_type == STD_VIDEO_AV1_FRAME_TYPE_SWITCH || (pStd->frame_type == STD_VIDEO_AV1_FRAME_TYPE_KEY && pic_info->showFrame)) ? 1 : u(1) & 0x1;
    }
    ...
    pic_flags->disable_cdf_update = u(1) & 0x1;
    if (sps->seq_force_screen_content_tools == STD_VIDEO_AV1_SELECT_SCREEN_CONTENT_TOOLS) {
        pic_flags->allow_screen_content_tools = u(1) & 0x1;
    } else {
    ...
    if (pic_flags->allow_screen_content_tools) {
        if (sps->seq_force_integer_mv == STD_VIDEO_AV1_SELECT_INTEGER_MV) {
            pic_flags->force_integer_mv = u(1) & 0x1;
        }
        else {
    ...
    if (!sps->flags.reduced_still_picture_header) {
        ...
        pic_flags->frame_size_override_flag = pStd->frame_type == STD_VIDEO_AV1_FRAME_TYPE_SWITCH ? 1 : u(1) & 0x1;
        //order_hint
        pStd->OrderHint = sps->flags.enable_order_hint ? u(sps->order_hint_bits_minus_1 + 1) & 0x1 : 0;
    ...
    if (sps->decoder_model_info_present) {
        pic_flags->buffer_removal_time_present_flag = u(1) & 0x1;
    ...
    if (IsFrameIntra()) {
        ...
        if (pic_flags->allow_screen_content_tools && frame_width == upscaled_width) {
            pic_flags->allow_intrabc = u(1) & 0x1;
        }
        ...
        {
            if (sps->flags.enable_order_hint) {
                pic_flags->frame_refs_short_signaling = u(1) & 0x1;
            } else {
                pic_flags->frame_refs_short_signaling = 0;
            }
            ...
            if (pic_flags->force_integer_mv) {
                ...
                pic_flags->allow_high_precision_mv = u(1) & 0x1;
            }
    ...
    if (!(sps->flags.reduced_still_picture_header) && !(pic_flags->disable_cdf_update)) {
        pic_flags->disable_frame_end_update_cdf = u(1) & 0x1;
    } else {
    ...
    if (!IsFrameIntra()) {
        pic_flags->reference_select = u(1) & 0x1;
    } else {
        pic_flags->reference_select = 0;
    }

    pic_flags->skip_mode_present = IsSkipModeAllowed() ? u(1) & 0x1 : 0;

    if (!IsFrameIntra() && !pic_flags->error_resilient_mode && sps->flags.enable_warped_motion) {
        pic_flags->allow_warped_motion = u(1) & 0x1;
    } else {
        pic_flags->allow_warped_motion = 0;
    }

    pic_flags->reduced_tx_set = u(1) & 0x1;
